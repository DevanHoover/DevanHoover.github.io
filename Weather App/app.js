...
async function getWeather() {
    try {
        ...
        const temp = data.hourly.temperature_2m[0].toFixed(0) + '°';
        const humidity = data.hourly.relative_humidity_2m[0];

        const precipitation = data.hourly.precipitation[0].toFixed(1);
        if(precipitation == null || precipitation == undefined) {
            precipitation = '0%';
        }
        const visibility = (data.hourly.visibility[0] * 0.000621371).toFixed(0); // converts meters to miles 

        const weatherType = data.hourly.weather_code[0];
